...
export default function VoiceOverlay() {
  ...
  useSpeechRecognitionEvent("result", (event) => {
    if (event.isFinal) {
      ...
      setInterimText("");
      if (transcript.trim()) {
        process(transcript.trim());
      } else {
        process("Could not recognize speech");
      }
      setVstate("thinking");
    } else {
  ...
  const handleMic = async () => {
    if (vstate === "listening") {
      // Stop recognition — result/end events will fire automatically
    ...
    // Request permission and start on-device recognition
    const { granted } =
    ...
    let sttLang = "en-US";
    if (lang === "ne" || lang === "auto")
      sttLang = "ne-NP"; // default auto to Nepali to test detection
    else if (lang === "rom") sttLang = "en-IN";
        ...
        <SafeAreaView style={{ flex: 1 }}>
          ...
          <View style={styles.topBar}>
            ...
            >
              ...
              <Text style={[styles.langBtnText, { color: theme.ink }]}>
                {LANG_LABELS[lang]}
              </Text>
            </TouchableOpacity>
            {langMenuOpen && (
                  ...
                  >
                    <Text
                      style={[
                        styles.langRowText,
                        {
                          color: lang === o.k ? theme.cta : theme.ink,
                          fontWeight: lang === o.k ? "700" : "500",
                        },
                      ]}
                    >
                      {o.label}
            ...
            <View style={styles.topRight}>
              ...
              <TouchableOpacity
                style={[styles.iconBtn, { backgroundColor: theme.tile }]}
                onPress={closeVoice}
              >
          ...
          {historyOpen && (
            <View style={[styles.historyPanel, { backgroundColor: theme.ov }]}>
              ...
              >
                ...
                <Text style={[styles.histTitle, { color: theme.ink }]}>
              ...
              >
                {messages.length === 0 ? (
                  <Text style={[styles.emptyText, { color: theme.muted }]}>
                    No conversation yet — tap the orb and speak.
                  </Text>
                ) : (
                  messages.map((m) => (
                    <View
                    ...
                    >
                      {m.role === "bot" && (
                        <Text
                          style={[styles.bubbleName, { color: theme.muted }]}
                        >
                          {persona}
                        </Text>
                      )}
          ...
          {settingsOpen && (
            <>
              ...
              >
                <View
                  style={[styles.sheetHandle, { backgroundColor: theme.line }]}
                />
                <Text style={[styles.settTitle, { color: theme.ink }]}>
                  Voice settings
                </Text>

                <Text style={[styles.settSection, { color: theme.muted }]}>
                  Voice persona
                </Text>
                  ...
                  {(["Asha", "Kiran"] as const).map((p) => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.personaBtn,
                        {
                          borderColor: persona === p ? theme.cta : theme.line,
                          backgroundColor:
                            persona === p
                              ? "rgba(185,197,245,0.12)"
                              : "transparent",
                    ...
                    >
                      <Text style={[styles.personaLabel, { color: theme.ink }]}>
                        {p}
                      </Text>
                      <Text style={[styles.personaSub, { color: theme.ink }]}>
                        {p === "Asha"
                          ? "Warm, higher voice"
                          : "Calm, deeper voice"}
                      </Text>
                  ...
                  {[
                    ...
                    <TouchableOpacity
                      ...
                      style={[
                        styles.speedBtn,
                        {
                          borderColor: speed === o.v ? theme.cta : theme.line,
                          backgroundColor:
                            speed === o.v
                              ? "rgba(185,197,245,0.12)"
                              : "transparent",
                    ...
                    >
                      <Text style={[styles.speedText, { color: theme.ink }]}>
                        {o.l}
                      </Text>
                    </TouchableOpacity>
                  ))}
                ...
                <View style={styles.themeRow}>
                  ...
                  <TouchableOpacity
                    style={[
                      styles.toggle,
                      { backgroundColor: isDark ? theme.tile : theme.cta },
                    ]}
                    onPress={toggleTheme}
                  >
                    <View
